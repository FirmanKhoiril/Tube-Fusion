...
const CryptoDetail = () => {
  ...
  <div className="flex flex-col p-4 rounded-lg shadow-md bg-[#1e1d24] gap-6">
  ...
  <div className="flex w-full justify-between">
    ...
    <div className="flex gap-3">
      ...
      <button type="button" onClick={() => setLikeCoin((prev) => !prev)} name={`Love ${detail.name}`} className="py-2 max-w-[44px]  w-full px-3 bg-[#2c2b33] flex items-center gap-2 text-white rounded-md hover:bg-[#33323b]">
        {likeCoin ? <GoHeartFill size={24} /> : <GoHeart size={24} />}
      </button>
    </div>
  </div>
</div>
  ...
  <div className="w-full bg-[#1e1d24] p-4 rounded-lg shadow-md h-full">
    <div className="flex w-full justify-between items-center mb-4">
      <div className="flex items-center text-sm sm:text-base gap-4">
        ...
        <p>Current {detail.name} Price: ${millify(detail.price)}</p>
      </div>
